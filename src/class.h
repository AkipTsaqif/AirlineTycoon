...
//--------------------------------------------------------------------------------------------
//Variablen, die immer im Speicher herumspringen und daher nicht gefa�t werden k�nnen:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Eine Variable im Zeitverlauf f�r die Statistik: (siehe Sim.cpp)
//--------------------------------------------------------------------------------------------
class /**/SValue
{
   private:
...
//--------------------------------------------------------------------------------------------
//Ein CWait-Objekt f�r den Waitcursor der Maus:
//--------------------------------------------------------------------------------------------
class /**/CWait
{
   public:
...
//--------------------------------------------------------------------------------------------
//Eine Markierung f�r Clipping des Horizontes
//--------------------------------------------------------------------------------------------
class /**/CClipMarker
{
   public:
...
//--------------------------------------------------------------------------------------------
//Eine Markierung f�r den Cursor
//--------------------------------------------------------------------------------------------
class /**/CAreaMarker
{
   public:
...
//--------------------------------------------------------------------------------------------
//Eine T�r im Flughafen:
//--------------------------------------------------------------------------------------------
class /**/CDoor
{
   public:
      XY             ArrayPos;      //Ausl�ser im Boden
      SLONG          Winkel;
      UBYTE          State;
      SLONG          Dir;
      BOOL           ArabDoor;
...
//--------------------------------------------------------------------------------------------
//Alles was an der Tafel bei der Flugleitung aush�ngt: (in Tafel.Cpp)
//--------------------------------------------------------------------------------------------
class /**/CTafelZettel
{
   public:
...
class /**/CTafelData
{
   ...
   public:
      void Clear (void);            //Daten alle l�schen ==> keine Zettel
      void Randomize (SLONG Day);   //Zettel f�r n Tage auslosen
...
class /**/CGates
{
   public:
      FBUFFER<CGate> Gates;
      UBYTE          Auslastung[24*7]; //F�r 2x alle 24 Stunden der Uhr: Wie viele sind belegt?
      SLONG          NumRented;
...
class /**/CPlaneType
{
   ...
   public:
      ...
      __int64  NotizblockPhoto;     //Photos f�r den Notizblock
      SLONG    AnzPhotos;           //Zahl der Photos
      SLONG    FirstMissions;       //Ist erst verf�gbar ab Mission x
   ...
   public:
      ...
      SLONG    Passagiere;          //Maximale Zahl der Passagiere (ein erste Klasse Passagier verbraucht 2 Pl�tze)
      ...
      float    Wartungsfaktor;      //Faktor f�r die Wartungskosten
      CString  Kommentar;           //Ggf. allgemeines �ber diese Maschine
...
//--------------------------------------------------------------------------------------------
//Der Auftrag f�r einen Flug: (Auftrag.cpp)
//--------------------------------------------------------------------------------------------
class /**/CAuftrag
{
   ...
   public:
      ...
      ULONG          Personen;            //So viele m�ssen in die Maschine passen
      UWORD          Date;                //Vertragsbeginn
      UWORD          BisDate;             //Vertragsende
      SBYTE          InPlan;              //0=Nix, -1=Durchgef�hrt, 1=1x im Plan
      SBYTE          Okay;                //0=Nix, -1=Durchgef�hrt, 1=1x im Plan
      SLONG          Praemie;             //Pr�mie bei Erf�llung
...
//--------------------------------------------------------------------------------------------
//Der Frachtauftrag f�r einen Flug: (Fracht.cpp)
//--------------------------------------------------------------------------------------------
class CFracht
{
   public:
      ULONG          VonCity;             //bezeichnet eine Stadt
      ULONG          NachCity;            //bezeichnet eine Stadt
      SLONG          Tons;                //So viele Tonnen m�ssen insgesamt geflogen werden
      SLONG          TonsOpen;            //So viele Tonnen sind weder geflogen, noch im Flugplan verplant
      SLONG          TonsLeft;            //So viele Tonnen m�ssen noch geflogen werden
      UWORD          Date;                //Vertragsbeginn
      UWORD          BisDate;             //Vertragsende
      SBYTE          InPlan;              //0=Nix, -1=Durchgef�hrt, 1=1x im Plan
      SBYTE          Okay;                //0=Nix, -1=Durchgef�hrt, 1=1x im Plan
...
class CRoute
{
   public:
      ...
      ULONG          NachCity;            //bezeichnet eine Stadt
      SLONG          Miete;
      double         Faktor;              //Attraktivit�t der Route
...
class CRentRoute
{
   public:
      ...
      SLONG          HeuteBefoerdert;     //Soviele Passagiere haben wir heute hier bef�rdert
...
{
   public:
      UBYTE          Okay;                //0=Auftrag Okay 1=falscher Tag, 2=schon durchgef�hrt, 3=Passagiere passen nicht
      UBYTE          HoursBefore;         //So viele Stunden vor dem Start wurde der Flug festgelegt
      UWORD          Passagiere;          //Zahl der belegten Sitzpl�tze (Normal)
      UWORD          PassagiereFC;        //Zahl der belegten Sitzpl�tze (in der ersten Klasse)
      UWORD          PArrived;            //Zahl Passagiere, die schon im Flughafen sind
      SLONG          Gate;                //-1 = kein Gate frei ==> Flugfeld; -2=externer Hafen
      UBYTE          GateWarning;         //Warnung, da� ein anderer Flug zu dieser Zeit Probleme macht
      ULONG          VonCity;             //bezeichnet eine Stadt
      ULONG          NachCity;            //bezeichnet eine Stadt
      ...
      SLONG          Ticketpreis;         //Ticketpreis f�r Routen
      SLONG          TicketpreisFC;       //Ticketpreis f�r Routen (Erste Klasse)
...
class CFlugplan
{
   public:
      ...
      SLONG NextFlight;                   //Aktueller; sonst n�chster Flug
      SLONG NextStart;                    //Verweis auf den n�chsten, startenden Flug
...
class /**/CPlane
{
   ...
   public:
      ...
      BOOL           BehindGlobe;         //Ist das Flugzeug auf der R�ckseite der Erde
      ...
      FBUFFER<ULONG> Salden;              //Die t�glichen Einnahmen-Ausgaben des Flugzeuges der letzten Woche
      ...
      //SLONG          AnzPutzcrew;
      SLONG          Auslastung;          //Zu soviel % ist es gef�llt
      SLONG          AuslastungFC;        //Zu soviel % ist es gef�llt
      SLONG          Kilometer;           //Soviele Kilometer ist es schon geflogen
      SLONG          SummePassagiere;     //Soviele Passagiere hat es schon bef�rdert
      SLONG          MaxPassagiere, MaxPassagiereFC; //Soviele Leute passen bei der derzeiten Konfiguration rein
      BOOL           Sponsored;           //Wenn TRUE, dann ist es staatlich gef�rdert und schnlecht zu verkaufen
      BOOL           OhneSitze;           //Wenn TRUE, dann wurden die Sitze zum transportieren von Fracht ausgebaut (Umr�stkosten)
   ...
   public:
      ...
      SLONG    ptPassagiere;          //Maximale Zahl der Passagiere (ein erste Klasse Passagier verbraucht 2 Pl�tze)
      ...
      float    ptWartungsfaktor;      //Faktor f�r die Wartungskosten
      CString  ptKommentar;           //Ggf. allgemeines �ber diese Maschine
...
//--------------------------------------------------------------------------------------------
//Die Basisdaten f�r die Klackertafel:
//--------------------------------------------------------------------------------------------
class /**/CKlackerPlane
{
   public:
...
class /**/KLACKER
{
   public:
      ...
      char            Soll [24*16];  //So sollen die Pl�ttchen zur Zeit sein
      char            Haben [24*16]; //So sind die Pl�ttchen zur Zeit (Indices auf FontDef)
      SBFX            KlackerFx[3];
...
{
   public:
      CString Headline;                //Die �berschrift
...
class /**/HEADLINES
{
   private:
      ...
      CHeadline Override [30];     //Wird tags�ber gesetzt und am n�chsten Tag effektiv

      //Flexibles Zufallszeux:
      CString   FlexiCity[3];      //Zufallsstadt f�r einen Thread
      SLONG     FlexiNumber[3];    //Zufallszahl f�r einen Thread
...
class /**/CITY //Eine Stadt
{
   public:
      ...
      SLONG       TextRes;          //Base-Ressource Id f�r Texte
      SLONG       AnzTexts;         //Anzahl der Seiten mit Text
      CString     PhotoName;        //Name des Photos auf die Stadt
      SLONG       AnzPhotos;        //Anzahl der Photos (%li mu� dann im Namen vorkommen)
      ...
      SLONG       BuroRent;         //Die Monatsmiete f�r eine Niederlassung
      BOOL        bNewInAddOn;      //Ist im Add-On neu hinzugekommen?
      //Vorraussetzung f�r Anflug
...
{
   public:
      CString                 Title;               //Die �berschrift der Tabelle
      SLONG                   AnzRows;             //Zahl der Zeilen
      SLONG                   AnzColums;           //Zahl der Spalten
      FBUFFER<CString>        ColTitle;            //�berschriften der Spalten
      FBUFFER<CString>        Values;              //Die Werte der Tabellenfelder
      FBUFFER<UBYTE>          ValueFlags;          //Zusatzangaben f�r Tabellenfelder
...
class /**/BLOCK
{
   public:
      ...
      CPlaner   *Base;              //Basisdaten f�rs malen
      GfxLib    *pGLibPicture;
      XY         ScreenPos;         //Position auf dem Bildschirm
                 
      BOOL       Destructing;       //Block wird gerade zerst�rt
      SLONG      AnimationStart;    //Startzeit f�r die Animation
      SLONG      PlayerNum;         
      ...
      UBYTE      Indexes[6];        //Die Zust�nde f�r verschiedene Bereiche
      SLONG      SelectedIds[6];    //Die Zust�nde f�r verschiedene Bereiche
...
class /**/BRICK //Ein einzelnes Bodenteil eines bestimmten Zeitalters
{
   private:
      ...
      SLONG               RamPriority;   //Priotit�t f�rs VGA-RAM
      BOOL                NonTrans;      //Flag falls ohne Glass
      BOOL                Triggered;     //L�uft die Animation nicht st�ndig, sondern getriggert ab?
      BYTE                Layer;         //Die Sortierungsschicht (y �bergeordnet)
      BYTE                AnimSpeed;     //Divisor f�r Ticker als Bitmap-Index
      SLONG               FloorOffset;   //Y-Offset f�r die Sortierung
      XY                  BaseOffset;    //Basis f�r iEditor
      XY                  Grid;          //Schrittweiter f�r iEditor
      SLONG               MinY;          //Limit f�r iEditor
      SLONG               MaxY;          //Limit f�r iEditor
...
class /**/BUILD //Die Verwendung eines Bodenteils
{
   private:
      ...
      UBYTE  Par;             //Parameter, z.B. f�r die Raumnummer
...
class /**/CLAN
{
   private:
      ...
      SLONG          Group;            //Gruppe, die dieser Clan angeh�rt
      UBYTE          Wkeit;            //Wkeit, da� nicht neu gew�rfelt wird
      UBYTE          UpdateNow;        //Ist ein Pool-Update erw�nscht?
      SLONG          WalkSpeed;        //Laufgeschwindigkeit
      SLONG          Faktor;           //Animationsgeschwindigkeit
      XY             Offset;           //Offset zum blitten
      XY             ShadowOffset;     //Offset zum blitten f�r den Schatten
      XY             SkelettOffset;    //Offset zum blitten f�r das Skelett
      XY             GimmickOffset;    //Offset zum blitten f�r das Gimmick
      CString        PalFilename;      //Hierdrin ist die Palette zu finden
      SLONG          GimmickArt1;      //Art des Gimmicks (LookDir) wenn Person l�uft
...
//--------------------------------------------------------------------------------------------
// Die Auspr�gungen der Clan, die gerade so am Flughafen rumlaufen: (Person.cpp)
//--------------------------------------------------------------------------------------------
class /**/PERSON
{
   private:
      ...
      SLONG       FloorOffset;      //Cache f�r den Clan-Wert
      UBYTE       WaitCount;        //Wegen Deadlock; wird beim Warten erh�ht (20x pro sec); Bei den Werten 200-250 wird gedr�ngelt
      ...
      TEAKRAND    PersonalRandWalk; //Der private Zufallsgenerator f�r Laufangelegenheiten
...
class /**/CQueuedPerson
{
   private:
      ...
      SLONG       Priority;         //Zuf�llige Priorit�t, falls mehrere Personen erscheinen sollen (durch Netzwerk-Konflikte)
...
class /**/HISTORY
{
   ...
   public:
      ...
      void AddNewCall (SLONG Type);                         //Bit 0: Handy, Bit 1+2: Ortgespr�ch, Ferngespr�ch, Auslandsgespr�ch
...
class CTalker
{
   public:
      SLONG    OwnNumber;
      SLONG    State;      //0=nix, 1=Gespr�ch, 2=Telefonat annehmen, 3=Telefonieren, 4=Auflegen
   ...
   public:
      ...
      void IncreaseReference (void);  //F�rs laden/rauswerfen der Bitmap
      void DecreaseReference (void);
      void IncreaseLocking (void);    //F�rs sperren
...
//--------------------------------------------------------------------------------------------
// Smacker 16-Bit Interface f�r den Flughafen:
//--------------------------------------------------------------------------------------------
...
class PLAYER
{
   ...
   public:
      ...
      __int64          Bonus;      //versteckter Bonus, den der Computerspieler noch erh�lt
      ...
      SLONG          GameSpeed;  //0..3
      SLONG          ArabTrust;  //Sabotage m�glich?
      ...
      SLONG          ArabActive; //FALSE=Flugzeug war noch nicht gelandet; TRUE=Flugzeug war am Boden, Anschlag jetzt m�glich
      ...
      BOOL           GlobeOiled; //Ist der Globus ge�lt?
      SLONG          MechTrust;  //0=kein, 1=offen, 2=genehmigt
      SLONG          MechAngry;  //0=nein, 1=w�tend
      UWORD          EarthAlpha;
      ...
      BOOL           RunningToToilet;   //Mu� er ganz dringend zur Toilette?
      ...
      CSmack16      *pSmack;            //Video f�r die Explosion
      BUFFER<CSmoke> Smoke;
      SLONG          LaptopVirus;       //0=kein Virus, 1=Virus, 2=In Reparatur, 3=abholbereit
      SLONG          WerbeBroschuere;   //-1=keine Brosch�re, sonst die vom Spieler
      BOOL           TelephoneDown;     //0=Telefone okay, 1=Telefon ausgefallen
      BOOL           Presseerklaerung;  //1=Ist heute Opfer einer Presseerkl�rung
      BOOL           GlobeFileOpen;     //Ist das Filofax im Globus offen?
      BOOL           HasAlkohol;        //Alkohol im Glas im B�ro drin?
      ...
      SLONG          StrikePlanned;     //TRUE ==> Worker gehen zum n�chsten fairen Termin auf Streik
      SLONG          StrikeEndCountdown;//In soviel Minuten wird der Streik jetzt beendet
      SLONG          StrikeEndType;     //Und zwar auf die Art: 1=Drohung; 2=Gehaltserh�hung; 3=Trinker; 4=Abwarten
      ...
      SLONG          SeligTrust;        //Bei Fr�ulein Selig eingeschleimt?
      ...
      BOOL           bReadyForMorning;  //Ist Spieler bereit (wichtig f�r Netzwerk)
      BOOL           bReadyForBriefing; //Ist Spieler an Folgetagen bereit f�r's Briefing

   //Statistik/Spielziel:
   public:
      BOOL           HasFlownRoutes;    //Flag, ob er schon einmal eine Route geflogen ist
      SLONG          NumAuftraege;      //Zahl der geflogenen Auftr�ge
      SLONG          NumPassengers;     //Zahl der Passagieren, die man bef�rdert hat
      __int64        Gewinn;
      SLONG          ConnectFlags;      //Flags f�r St�dte, die verbunden wurden
      SLONG          RocketFlags;       //Flags f�r die Raketenbauteile
      ...
      SLONG          NumOrderFlights;   //Soviele von Uhrigs Auftr�gen ist man geflogen
      SLONG          NumOrderFlightsToday;   //Soviele von Uhrigs Auftr�gen ist man heute geflogen
      SLONG          NumOrderFlightsToday2;  //Soviele von Uhrigs Auftr�gen hat der Computer heute angenommen
   ...
   public:
      ...
      CAuftraege     Auftraege;  //Vertr�ge die er f�r Fl�ge abgeschlossen hat
      CFrachten      Frachten;   //Vertr�ge die er f�r Fl�ge abgeschlossen hat
      CGates         Gates;      //Die Gates (immer CheckIn + Abflug) die gemietet wurden
      FBUFFER<UBYTE> Items;      //Dinge die der Spieler besitzt
      SLONG          LaptopBattery;
      SLONG          LaptopQuality;
      CRentCities    RentCities; //Die er�ffneten Niederlassungen
      ...
      BLOCKS         Blocks;     //Die Notizbl�cke
      CMessages      Messages;   //Die Berater
      CLetters       Letters;    //Die Briefe im Postkorb

   //Aktien & Zinsen:
   public:
      UBYTE          SollZins;   //Zins f�r Schulden
      UBYTE          HabenZins;  //Zins f�r Guthaben
   ...
   public:
      ...
      XY             IslandViewPos; //SichtPosition f�r die Inseln
      XY             CameraSpeed;   //Tr�gheitssteuerung. Dies ist die Geschw. der Kamera
      CStdRaum      *LocationWin;//Pointer auf offenes Sub-Fenster (Raum)
      CStdRaum      *DialogWin;  //Pointer auf offenes Dialog-Sub-Fenster
      UBYTE          NewDir;     //Flag, falls die Laufrichtung ge�ndert wurde
      XY             WinP1;      //Die Position des ViewFensters im GameFrame Fenster
      XY             WinP2;
      UWORD          Locations[10];//0=kein, 1=Flughafen; 2-? = Raum
      SLONG          LocationTime; //Seit diesem Sim.Time sind wir in dieser Location (oder -1 f�r Flughafen)
      ...
      UWORD          TopLocation;
      BYTE           Windows;    //Bits 1, 2, 4, 8 f�r die belegten Ausschnitte
   ...
   public:
      ...
      XY             SecondaryTarget;  //f�r Raumeing�nge
      XY             TertiaryTarget;   //f�r Treppen
   ...
   //Computerspieler
   public:
      BOOL           ImageGotWorse;    //F�r's Briefing
      FBUFFER<CRobotAction> RobotActions;
      SLONG          StandStillSince;  //TimeOut
      SLONG          LastActionId;
      SLONG          WorkCountdown; //Computer "macht" etwas
      SLONG          WaitWorkTill;  //Timeslice-Angabe, wann der Computerspieler seine Aktion beginnen soll; Wichtig, damit im Netzwerk die Aktionen absolut synchron ausgef�hrt werden
      ULONG          WaitWorkTill2; //Sim.Time-Angabe, nur zweitrangig f�r die Synchronisierung
      SLONG          TimeBuro;      //Wann war der Computerspieler zuletzt
      SLONG          TimePersonal;  //im B�ro, beim Reiseb�ro, �.�.
      ...
      FBUFFER<SLONG> Sympathie;     //Sympatiewerte f�r die Spieler
      ...
      SLONG          OutOfGates;    //Mu�te gepatcht werden?
      ...
      SLONG          SpeedCount;    //Trick f�r den Rundenwechsel zu beschleunigen
   ...
   public:
      ...
      void  CalcRoom (void);              //Speed-up f�r GetRoom
      SLONG CalcPlanePropSum (void);      //Berechnet, was die anstehenden Umr�stungen zusammen kosten werden
      void  ChangeMoney (__int64 Money, SLONG Reason, CString Par1, char *Par2=NULL);  //�ndert Barschaft und Profit
      ...
      UWORD GetRoom (void);                         //Gibt den aktuellen Raum zur�ck
      SLONG GetMissionRating (bool bAnderer=false); //Gibt aktuellen Missionserfolg als Zahl zur�ck
      ...
      void  LeaveRoom (void);             //Verl��t der Raum
      void  LeaveAllRooms (void);         //Verl��t alle R�ume
      void  MapWorkers (BOOL Advice);     //Verkn�pft worker mit Flugzeugen
      void  MapWorkerOverflow (BOOL Advice);
      void  UpdatePilotCount (void);
      void  NewDay (void);                //L��t neuen Tag & ggf. Monat beginnen
      void  PlanGates (void);
      ...
      void  UpdateAuftraege (void);       //Bringt Auftr�ge aus neusten Stand; bucht Strafen
...
class AIRPORT
{
   ...
   public:
      ...
      SLONG           NumBeltSpots;  //Zahl der Stehpl�tze beim Gep�ckband

   //Die Spezialinformationen (Positionsidiciert):
   public:
      XY             PlateOffset;         //Verschiebt die Build-Koordinaten in Plate-Koor.
      XY             PlateDimension;      //Ma�e des Arrays
      FBUFFER<UBYTE> iPlate;              //XY-Platten mit den Informationen 
      FBUFFER<UBYTE> iPlateDir;           //Kopie mit der Richtung einer Person, die dort l�uft
      FBUFFER<UWORD> SeatsTaken;          //X-Array, in Y-Achse die Sitze

   //Koordinaten von Sonderf�llen (Nummerierungsindiciert):
   public:
...
// Sim.Cpp :
//--------------------------------------------------------------------------------------------
class SIM //Die Simulationswelt; alles was zur aktuellen Partie geh�rt
{
   //Daten:
   public:
      ...
      SLONG          bExeChanged; //Wurde die Exe ver�ndert?
      DWORD          UniqueGameId;//Identifiziert eine Session eindeutig
      DWORD          UniqueGameId2;//Identifiziert eine Session eindeutig, ist auch ohne Netzwerk verf�gbar und wird im Spiel nicht ver�ndert (ist aber im Netzwerk nicht synchron)
   ...
   public:
      ...
      ULONG       DialogOvertureFlags;//Flags, f�r diverse Dialoge
      ...
      BOOL        IsTutorial;         //L�uft das Tutorial (mit der Sekret�rin noch?)
      SLONG       MaxDifficulty;      //Maximaler Schwierigkeitsgrad
      SLONG       MaxDifficulty2;     //Maximaler Schwierigkeitsgrad f�r Add-On
      SLONG       MaxDifficulty3;     //Maximaler Schwierigkeitsgrad f�r Add-On Fligh Security
      ULONG       GameSpeed;          //Time+=Gamespeed
      BOOL        bPause;             //Spiel im Pause-Modus
      BOOL        bNoTime;            //Zeit wird nicht weitergez�hlt (Briefing)
      BOOL        DayState;           //1=Tag wird gestartet; 2=Spieler spielt; 3=Tag wird beendet; 4=Tag wurde beendet
      BOOL        CallItADay;
      SLONG       CallItADayAt;        //Netzwerk: Feierabend bei dieser TimeSlice einl�uten
      ...
      SLONG       MoneyInBankTrash;    //Liegt bei der Bank Geld im M�lleimer  (-1=noch nicht, 1=ja, 0=nein, weil weg
      ...
      BOOL        ItemClips;           //Hat heute schon jemand die B�roklammern genommen?
      ...
      BOOL        ItemParfuem;         //Hat heute schon jemand das Parf�m genommen?
      BOOL        ItemZange;           //Hat heute schon jemand die Zange genommen?
      SLONG       nSecOutDays;         //0 oder n=So viele Tage f�llt das Security Office noch aus
      ...
      ULONG       KeyHints[3];         //Hilfe-texte f�r die Tasten: [0]=einmal [1]=heute [2]=zum zweiten Mal
      bool        bThisIsSessionMaster;//Ist dies der Server?

      FBUFFER<SLONG> MissionCities;    //Die Zielst�dte f�r die Missionen
   ...
   //Statistik:
   public:
	   bool		   StatfGraphVisible;							// true -> Der Graph ist sichtbar, ansonsten die schn�den Zahlen
	   bool		   StatplayerMask[4];							// Diese Spieler wurden zur Ansicht ausgew�hlt
	   BYTE		   Statgroup;										// Die angew�hlte Gruppe (*0=Finanzen, 1=?, 2=?)
	   SLONG	   Statdays;										// Anzahl der darzustellenden Tage
	   SLONG	   StatnewDays;									// F�r eine Animation
	   bool		   StatiArray[3][16];		               // Merkt sich f�r jede Gruppe welche Eintr�ge selektiert sind.
      SLONG       DropDownPosY;
   ...
   public:
      ...
      UBYTE       QuitCountDown; //Z�hler, die lange die Leute bis zum Quit winken
      DWORD       TickerTime;    //Z�hler f�r die AnimBricks
      SLONG       TimeSlice;     //F�r die Synchronisation im Netzwerk
      ...
      SLONG       Weekday;       //0-6 f�r Mo-So
      SLONG       Jahreszeit;    //0-3 = Fr�hling, Sommer, Herbst, Winter geladen, -1 noch keine Jahrezeit geladen

   //Eine eventuelle Aktien�bernahme:
      SLONG       OvertakenAirline;   //Konkurrenzlinie schlucken?
      SLONG       OvertakerAirline;   //Konkurrenzlinie schlucken?
      SLONG       Overtake;           //0=Nix, 1=Schlucken, 2=Aufl�sen

   //Sonstiges:
      SLONG       TickReisebueroRefill;  //Wird alle 5 Sekunden erh�ht und gibt an, wieviel max. gef�llt werden darf
      SLONG       TickLastMinuteRefill;
      SLONG       TickFrachtRefill;
      SLONG       TickMuseumRefill;

   //Die Ausbauzust�nde des Flughafens:
   public:
