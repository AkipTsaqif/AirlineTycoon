//============================================================================================
// Person.Cpp - Verwaltung von Clans und Personen (Clan-Auspr�gungen)
//============================================================================================
#include "stdafx.h"
#include "AtNet.h"
...
//--------------------------------------------------------------------------------------------
//Gibt das Vorzeichen (-1, 0 oder 1) des Parameters zur�ck:
//--------------------------------------------------------------------------------------------
...
void CLANS::ReInit (const CString &TabFilename)
{
   ...
   while (1)
   {
      for (SLONG ShadowPass=0; ShadowPass<3; ShadowPass++)
      {
         ...
         for (c=0; c<10+4; c++)
         {
            SLONG d;  //B�ser Patch, damit das Laufen erst ab Index 10 kommt, obwohl es in der Excel-Tabelle schon viel eher steht:
            if (c<4) d=c;
...
//--------------------------------------------------------------------------------------------
//L�dt den Zustand, welche Clans aktiv sind:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt den Id eines Kunden zur�ck:  0=nur wei�e; 1=beide; 2=nur braune
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt den Index einer entsprechenden Figur zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt den Index des entsprechenden koffers zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt den Index des erstbesten Clans mit diesem Typ zur�ck (gut f�r Player)
//--------------------------------------------------------------------------------------------
         ...
         if (IsInAlbum (c) && (*this)[c].Type<=11 && (*this)[c].Group<=0 && (*this)[c].HasSuitcase>=0)
         {
            if ((r--)==0)
            {
               ...
               if (!FirstDay)
               {
                  ...
                  {
                     n--;     //Wir haben schon genug Personen gel�scht/hinzugef�gt
                     break;
                  }
               }
...
PERSON::PERSON ()
{
   ...
   //15.11.2001: Hat aus irgendeineinem Grund Asynchronit�ten im Spiel verursacht. Darf zwar eigentlich nicht sein. War aber so:
...
PERSON::PERSON (UBYTE ClanId, XY Position, UBYTE Reason, UBYTE FlightAirline, SLONG FlightPlaneId, UBYTE FlightPlaneIndex, UBYTE Mood, UBYTE FirstClass)
{
   ...
   switch (PERSON::Reason)  //Sekund�rziel
   {            
      case 0:
      ...
      //Der Koffer f�hrt so durch die Gegend:
   ...
   if (Clans[(SLONG)ClanId].Type>=CLAN_PLAYER1 && Clans[(SLONG)ClanId].Type<=CLAN_PLAYER4)
   {
      ...
      //Au�erdem werden die IQ-Walk Variablen initialisiert:
...
void PERSON::DoOneCustomerStep (void)
{
   ...
   //Serge's Alte Sau (TM) steht nicht fr�h auf.
   ...
   //Ist die Person am Ziel ihrer Tr�ume? Dann weiter zum n�chsten Checkpoint!
   if (abs (Position.x-Target.x)<=StepSize.x && abs (Position.y-Target.y)<=StepSize.y)
      if ((State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG))!=PERSON_WAITING)
      {
         ...
         {
            PersonReachedTarget ();
            if (LookDir==4) //Gimmick-Animation f�r WayPoint-Figur?
            {
   ...
   //Hoppla! Sind wir etwa just �ber die Mittelline einer Bodenplatte gelaufen?
   if ((NewPos.x+4422)/44!=(Position.x+4422)/44 || (NewPos.y+2211)/22!=(Position.y+2211)/22 || Dir==4)
   {
      ...
      //Welche M�glichkeiten hat der Kunde dieses Feld zu verlassen?
      UBYTE NewPossibleDirs;
      
      //M�ssen wir erst einmal eine T�r aufsto�en?
      ...
      if ((State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG))==PERSON_WAITING)
      {
         ...
         if (ArrayPos.y>0 && ArrayPos.y<7)
         {
            ...
            {
               //Person setzt sich und schaut nach S�den:
      ...
      //Gibt es au�er der Herkunft nur einen Ausgang? Dann gehen wir dahin!
      ...
      DirtyWaypoingShortcut:
         ...
         {
            //Frisch aus der Nervenheilanstalt l�uft dieser Typ nur herum wie doof:
            ...
            //Person l�uft nerv�s im Warteraum hin und her:
            case PERSON_WAITING:
            ...
            //Eine Person l�uft innerhalb eines Ladens nur in x-richtung:
            ...
            //Person will zum n�chsten Waypoint:
            ...
            case PERSON_2SHOP:
               ...
               else
               {
                  ...
                  if (Position.x<Target.x) //nach rechts
                  {
                     ...
                     if (bFast && bAussenErlaubt && (ArrayPos.y+5==14 || (Airport.iPlate[13+((ArrayPos.x+1)<<4)]&2) && Target.x-Position.x>88))
                        ty = 209;    //Schnelle Clans �berholen au�en, wenn jemand vor ihnen ist...
                     else
                     {
                        ...
                        {
                           if (bAussenErlaubt)
                              ty = 209; //Jeder �berholt hier au�en...
                           else
                           {
                              //Pr�fen, ob auf der gemeinsamen Innenspur Platz ist...
                              for (c=0; c<5; c++)
                              ...
                              if (c==5)
                                 ty = 165; //alles frei, wir �berholen innen
                              else
                                 ty = 187; //normale Spur, weil jemand kommt
                           }
                        }
                        else //Nein, der Weg ist nicht blockiert, Aber ggf. behalten wir unsere �berholspur noch ein wenig...
                        {
                           if (ArrayPos.y+5==14 && Dir==1 && bAussenErlaubt && (Airport.iPlate[14+((ArrayPos.x+1)<<4)]&2)==0)
                              ty = 209; //Es ist Platz, also �berholen wir au�en...
                  ...
                  else if (Position.x>Target.x) //nach links
                  {
                     ...
                     if (bFast && bAussenErlaubt && (ArrayPos.y+5==10 || (Airport.iPlate[11+((ArrayPos.x-1)<<4)]&2) && Position.x-Target.x>88))
                        ty = 121;    //Schnelle Clans �berholen au�en, wenn jemand vor ihnen ist...
                     else
                     {
                        ...
                        {
                           if (bAussenErlaubt)
                              ty = 121; //Jeder �berholt hier au�en...
                           else
                           {
                              //Pr�fen, ob auf der gemeinsamen Innenspur Platz ist...
                              for (SLONG c=0; c<5; c++)
               ...
               //Versuchen wir nach unten zu gehen, obwohl wir nur nach links k�nnen?
               if (Dir==2 && (NewPossibleDirs&48)==16) Dir=3;

               //Versuchen wir nach links zu gehen, obwohl wir nur nach unten k�nnen?
               if (Dir==3 && (NewPossibleDirs&48)==32) Dir=2;
               break;

            //Person will nur raus aus dem Check-In Bereich und rennt daf�r auch andere �ber den Haufen...
...
DirtyLabel:
      ...
      //Sp�tenstens jetzt stimmt die Richtung:
      switch (Dir)
      {
         case 0: NewPos.y = Position.y - StepSize.y; break;
         case 1: NewPos.x = Position.x + StepSize.x; break;
      ...
      if (NewArrayPos.y>=0 && NewArrayPos.y<=9 && (State&PERSON_BROWSEFLAG)==0)
      {
         ...
         {
            //N�. Aber jetzt! (evtl. war da auch ein Mega-Stau...)
            Position = NewPos;
         ...
         {
            //Da ist schon wer; k�nnen wir ggf. nach links ausweichen?
            ...
            //Falls Frontalzusammensto� und wir nicht ausweichen k�nnen, dann gehen wir skrupellos durch den anderen durch:
...
void PERSON::DoOnePlayerStep (void)
{
   ...
   if (LookDir==4)
   {
      //W�hrend einer Explosion wird der Spieler ans Gel�nder gedr�ckt und kann sich nicht bewegen
      return;
   }
   else if (LookDir==8)
   {
   ...
   else
   {
      ...
      Position.y -= 5000;
      ArrayPos.x = Position.x/44;
      ArrayPos.y = (Position.y+2200)/22-100;  //21.2.02: war mal "Position.y/22", wegen Eingang Security-Raum ge�ndert
      Upfloor    = TRUE;
   }
   ...
   if ((NewPos.x+4422)/44!=(Position.x+4422)/44 || (NewPos.y+2211)/22!=(Position.y+2211)/22 || Dir==8)
   {
      ...
      if (Dir!=1 && Dir!=3 && Dir!=5 && Dir!=7)
      {
         ...
         Position.y = ((Position.y+2200) / 22) * 22 - 2200 + 11; //21.2.02: war mal "Position.y / 22 * 22 + 11", wegen Eingang Security-Raum ge�ndert
      }
      ...
      //Sind einige wom�glich f�r Spieler verboten?
      ...
      //L�uft der Spieler zur Zeit automatisch oder vom Mensch gesteuert?
      if (StatePar)
      {
      ...
      //Ja geht das denn mit der Richtung �berhaupt? (Bei Werkstatt �bersehen wir das 'mal)
         ...
         if (qPlayer.iWalkActive)
         {
            ...
            if (ArrayPos.y>=5 && ArrayPos.y<=14)
            {
               ...
               else
               {
                  ...
                  //W�rde in der Std-Ebene �berhaupt Platz sein?
      ...
      //Haben wir eine Raumank�ndigung betreten?
      if (StatePar == 0)
      {
         ...
         else if (c && (c<ROOM_REISE_X1 || c>ROOM_MONITOR6) && c!=ROOM_AIRPORT_EXIT && ((c!=ROOM_KIOSK && c!=ROOM_INSEL && c!=ROOM_SECURITY && c!=ROOM_RUSHMORE && c!=ROOM_WELTALL && c!=ROOM_RICKS && c!=ROOM_ROUTEBOX && c!=ROOM_SECURITY) || c==qPlayer.DirectToRoom))
         {
            ...
            //F�r 'echte' R�ume schauen wir auch das n�chste Ziel nach:
            if (StatePar<ROOM_STAIRSVON || StatePar>ROOM_STAIRSBIS)
            {
               ...
               //Doch nicht in einen Raum gehen, falls Dialog offen ist oder er�ffnet wird:
               ...
               else if ((Sim.RoomBusy[StatePar]>1 || qPlayer.LocationForbidden==StatePar) && !Sim.bNoTime && (StatePar<ROOM_BURO_A || StatePar>ROOM_PERSONAL_D))
               {
                  ...
                  //Nein, Standard-Raum, d.h. jetzt m�ssen wir vor die T�r:
                  ...
                  else if (qPlayer.Owner==1 && (Sim.bIsHost || Sim.bNetwork==0))
                  {
                     ...
                     //Roboter: Raum schon besetzt? Erst zweite Priorit�t ausf�hren:
   ...
   if (StatePar)
   {
      ...
      if (StatePar>=ROOM_STAIRSVON && StatePar<=ROOM_STAIRSBIS)
      {
         ...
         if (Upfloor) //AI f�r "oben sein"
         {
            //AI f�r "oben sein" & oben raus wollen
            ...
            //AI f�r "oben sein" & nach unten wollen
         ...
         else //AI f�r "unten sein"
         {
            //AI f�r "unten sein" & nach oben wollen
            if (StatePar==ROOM_STAIRS1UP || StatePar==ROOM_STAIRS2UP || StatePar==ROOM_STAIRS3UP)
            {
               Dir = 1;
               if ((Airport.iPlate[ArrayPos.y+(ArrayPos.x<<4)]&64)==0)
            ...
            //AI f�r "unten sein" & unten raus wollen
            else if (StatePar==ROOM_STAIRS1DOWN || StatePar==ROOM_STAIRS2DOWN || StatePar==ROOM_STAIRS3DOWN)
            {
               Dir = 5;
      ...
      {
         //Wir laufen autom. zu einem normalen Raum & daher m�ssen wir ggf. d. Position runden!
         if (abs(Position.x-Target.x)<=StepSize.x) Position.x=Target.x;
         if (abs(Position.y-Target.y)<=StepSize.y*2) Position.y=Target.y;

         Dir = 8;

         //Und die Richtung anschlie�end �berpr�fen:
         if (Position.x>Target.x && (StatePar==ROOM_WERKSTATT || (StatePar==-1 && Position.x>Airport.RightEnd-600))) Dir = 6;
         else if (Position.x>Target.x && (StatePar==ROOM_FRACHT || (StatePar==-1 && Position.x<Airport.LeftEnd+600))) Dir = 6;
         else if ((Position.y+1000)>((Target.y+1000)%5000)) Dir = 0;
         ...
         //Und nat�rlich �berpr�fen, ob wir angekommen sind (und nicht gerade raus wollen):
         if (StatePar!=-1)
         {
            c=Airport.GetRuneParNear (XY(NewPos.x,NewPos.y+Upfloor*5000), XY(StepSize.x,StepSize.y*2), RUNE_SHOP);
...
//--------------------------------------------------------------------------------------------
//Eine Person hat einen Checkpunkt (Sekund�rziel) erreicht und braucht neue Anweisungen:
//--------------------------------------------------------------------------------------------
void PERSON::PersonReachedTarget (void)
{
   //Das h�ngt davon ab, warum sie am Flughafen ist:
   switch (Reason)
   {
      ...
      case REASON_FLYING:
         {
            ...
            switch (State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG))
            {
               ...
               //Person ist jetzt am Flugzeug und v�llig verdeckt
               case PERSON_ENTERINGPL:
                  State = PERSON_LEAVING;
                  break;
...
//--------------------------------------------------------------------------------------------
//Gibt eine Referenz auf den aktuellen Flugplan zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Sortiert alle Personen gem�� ihrer Tiefe:
//--------------------------------------------------------------------------------------------
...
void PERSONS::DoOneStep (void)
{
   ...
   if (!Sim.CallItADay)
   {
      //Personen, die warten werden nur ber�cksichtigt wenn Ostern auf Pfingsten f�llt:
   ...
   else //schnelle Version t�r Tagesabbruch
   {
...
//--------------------------------------------------------------------------------------------
//Schaut, ob Stimmungen angezeigt werden m�ssen:
//--------------------------------------------------------------------------------------------
...
//Person-Queues:
//--------------------------------------------------------------------------------------------
//F�gt eine Person in die Queue ein:
//--------------------------------------------------------------------------------------------
void CPersonQueue::AddPerson (UBYTE ClanId, XY Position, UBYTE Reason, UBYTE FlightAirline, SLONG FlightPlaneId, UBYTE FlightPlaneIndex, UBYTE Mood, UBYTE FirstClass)
{
   ...
   //Suchen, ob an der Stelle erst k�rzlich jemand erschaffen wurde:
   for (c=0; c<Spots.AnzEntries(); c++)
   {
      ...
      if (Spots[c].Position==Position)
      {
         ...
         {
            if (d==Person.AnzEntries()-2) //Ggf. Array vergr��ern
            ...
            if (Person[d].TimeSlice==-1)
            {
               ...
               {
                  Spots[c].TimeSlice+=20;  //eine Sekunde sp�ter
...
void CPersonQueue::Pump (void)
{
   ...
   do
   {
      ...
      //Die Personenliste nach einer f�lligen Person durchsuchen:
      ...
      for (c=Person.AnzEntries()-1; c>=0; c--)
      {
         if (Person[c].TimeSlice!=-1 && Person[c].TimeSlice<=Sim.TimeSlice /*&& Person[c].Priority==MinPriority*/)
         {
            ...
            if (Sim.Persons.GetNumFree()<2) //Ggf. Array vergr��ern
