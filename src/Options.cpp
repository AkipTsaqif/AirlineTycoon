	...
	else if (!PreLButtonDown(point))
	{
		...
		case 5: //Laden:
			...
			if (Line >= 2 && Line <= 13 && SavegameInfos[Line - 2].GetLength() > 0)
			{
				...
				{
					//Netzwerk-Laden vom Hauptmen� aus:
				...
				{
					//Laden w�hrend des Spiels: Kommt das mit der aktuellen Anzahl der Spieler hin?
...
void Options::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (CursorY != -1)
	{
		...
		if (nChar == ' ' || nChar == '-' || nChar == '+' || nChar == '.' || (nChar >= 'A' && nChar <= 'Z') || nChar == '�' || nChar == '�' || nChar == '�' || (nChar >= '0' && nChar <= '9'))
		{
