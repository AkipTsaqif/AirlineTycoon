...
SLONG gNetworkSavegameLoading = -1;    //Komm-Variable, �ber die der Options-Screen mitteilt, welcher Spielstand f�r's Netzwerk geladen werden soll
...
void NewGamePopup::Konstruktor(BOOL bHandy, SLONG PlayerNum)
{
	...
	for (c = 0; c < Sim.Players.AnzPlayers; c++)
	{
		...
		{
			if (Sim.Players.Players[c].Name[d] == '�') Sim.Players.Players[c].Name.SetAt(d, '�');
			if (Sim.Players.Players[c].Name[d] == '�') Sim.Players.Players[c].Name.SetAt(d, '�');
			if (Sim.Players.Players[c].Name[d] == '�') Sim.Players.Players[c].Name.SetAt(d, '�');
		}
	}
...
void NewGamePopup::RefreshKlackerField(void)
{
	...
	else if (PageNum == PAGE_TYPE::MISSION_SELECT) //Mission w�hlen
	{
		CString tmp;

		KlackerTafel.PrintAt(0, 15, StandardTexte.GetS(TOKEN_NEWGAME, 4001)); //Zur�ck
	...
	else if (PageNum == PAGE_TYPE::ADDON_MISSION_SELECT) //Add-On Mission w�hlen
	{
		CString tmp;
	...
	else if (PageNum == PAGE_TYPE::FLIGHT_SECURITY_MISSION_SELECT) //Flight Security Mission w�hlen
	{
		CString tmp;
	...
	else if (PageNum == PAGE_TYPE::CAMPAIGN_SELECT)
	{
		...
		KlackerTafel.PrintAt(0, 6, StandardTexte.GetS(TOKEN_NEWGAME, 4001)); //Zur�ck
	}
	else if (PageNum == PAGE_TYPE::SELECT_PLAYER_SINGLEPLAYER || PageNum == PAGE_TYPE::SELECT_PLAYER_CAMPAIGN || PageNum == PAGE_TYPE::SELECT_PLAYER_MULTIPLAYER) //Spieler w�hlen
	{
		...
		KlackerTafel.PrintAt(0, 0, StandardTexte.GetS(TOKEN_NEWGAME, 999)); //Spieler w�hlen
	...
	else if (PageNum == PAGE_TYPE::MULTIPLAYER_SELECT_NETWORK) //Netzwerk: Provider-Medium w�hlen
	{
	...
	else if (PageNum == PAGE_TYPE::MULTIPLAYER_SELECT_SESSION) //Netzwerk: Session ausw�hlen / erzeugen
	{
	...
	else if (PageNum == PAGE_TYPE::SETTINGS_CHOOSE_AIRPORT) //Heimatflughafen w�hlen
	{
		KlackerTafel.PrintAt(0, 0, StandardTexte.GetS(TOKEN_NEWGAME, 600));    //Heimatflughafen w�hlen
	...
	{
		KlackerTafel.PrintAt(12 - (strlen(StandardTexte.GetS(TOKEN_NEWGAME, 530)) - 3) / 2, 8, StandardTexte.GetS(TOKEN_NEWGAME, 530));  //Zur�ck
	}
}

//--------------------------------------------------------------------------------------------
//�berpr�ft ob die Namen von Spielern & Fluggesellschaften eindeutig sind:
//--------------------------------------------------------------------------------------------
void NewGamePopup::CheckNames(void)
{
	...
	//Und es mu� EXAKT einen Human-Player geben:
	for (c = d = 0; c < 4; c++)
...
void NewGamePopup::OnPaint()
{
	...
	if (bActive)
	{
		...
		if (!IsDialogOpen() && !MenuIsOpen())
		{
			...
			else if (PageNum == PAGE_TYPE::MULTIPLAYER_SELECT_SESSION)  //Netzwerk: Session w�hlen
			{
...
void NewGamePopup::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == 196 || nChar == 228) nChar = (UINT)'�';
	if (nChar == 214 || nChar == 246) nChar = (UINT)'�';
	if (nChar == 220 || nChar == 252) nChar = (UINT)'�';
	...
	{
		if (nChar == '-' || nChar == ' ' || (nChar >= 'A' && nChar <= 'Z') || nChar == '�' || nChar == '�' || nChar == '�' || nChar == '.')
		{
			if (CursorX < 0)
			{
				if (nChar != ' ')
