...
//--------------------------------------------------------------------------------------------
//Sorgt f�r Screen Refresh und f�r Ablauf der Simulation:
//--------------------------------------------------------------------------------------------
SIM::SIM()
{
...
//--------------------------------------------------------------------------------------------
// F�gt einen Smacker im Flughafen hinzu:
//--------------------------------------------------------------------------------------------
               ...
               if (qFPE.ObjectType==1 || qFPE.ObjectType==2)
               {
                      ...
                      qFPE.VonCity==(ULONG)Sim.HomeAirportId)
                  {
                     for (n=0; n<2; n++)
                     {
                        ...
                        {
                           //Endlich werden Leute hinzugef�gt:
...
//------------------------------------------------------------------------------
//F�gt dem ganzen eine Stinkbombe hinzu:
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//F�gt dem ganzen etwas Klebstoff hinzu:
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//L��t den Benutzer Schwierigkeit, Anfangsjahr und Spieler w�hlen:
//------------------------------------------------------------------------------
void SIM::ChooseStartup (BOOL GameModeQuick)
{
   ...
   //Wochentag f�r die �ffnungszeiten:
   ...
   for (c=0; c<Sim.Players.AnzPlayers; c++)
   {
      ...
      //Die Flugzeuge kaufen:
      {
         ...
         switch (Sim.Difficulty)
         {
            case DIFF_EASY: //Mission: 2500 Passagiere bef�rdern
      ...
      if (qPlayer.Owner == 0) //20 Blocks f�r Spieler
      {
         Sim.localPlayer=c;
   ...
   //Aush�nge f�r Versteigerungen, Auftr�ge, ..:
...
//----------------------------------------------------------------------------------------
//Sucht die St�dte aus, die der Spieler in der Routenmissionen verbinden mu�
//----------------------------------------------------------------------------------------
void SIM::CreateMissionCities (void)
{
   ...
   //Die Map-Tabelle ist ein Filter, der alle St�dte nach ihrer Einwohnerzahl sortiert:
   ...
   //Default-St�dte:
   MissionCities[0]=Cities.GetIdFromNames ("R�o de Janeiro", "Rio de Janeiro", NULL);
   MissionCities[1]=Cities.GetIdFromNames ("Nova Iorque", "Nueva York", "New York", NULL);
   MissionCities[2]=Cities.GetIdFromNames ("T�quio", "Tokio", "Tokyo", NULL);
   MissionCities[3]=Cities.GetIdFromNames ("Deli", "Delhi", "Dill�", NULL);
   MissionCities[4]=Cities.GetIdFromNames ("Joanesburgo", "Johanesburgo", "Johannesburg", NULL);
   MissionCities[5]=Cities.GetIdFromNames ("Moskau", "Moskou", "Moscovo", "Moscou", "Mosca", "Mosc�", "Moscow", "Moskva", NULL);

   /*if (gLanguage==LANGUAGE_S) MissionCities[0]=Cities.GetIdFromName ("R�o de Janeiro");
   ...
   if (gLanguage==LANGUAGE_O)      MissionCities[2]=Cities.GetIdFromName ("T�quio"); //Portugisisch
   ...
   else if (gLanguage==LANGUAGE_S) MissionCities[5]=Cities.GetIdFromName ("Mosc�");
   ...
   //Wenn die notwendigen Routen normalerweise nicht existieren, m�ssen wir sie halt erzeugen:
   for (c=0; c<6; c++)
   {
      for (d=Routen.AnzEntries()-1; d>=0; d--)
         if (Routen.IsInAlbum(d))
...
//----------------------------------------------------------------------------------------
//Z�hlt die Uhr einen Schritt weiter und l�st ggf. Ereignisse aus.
//----------------------------------------------------------------------------------------
void SIM::DoTimeStep (void)
{
   ...
   if (Minute!=OldMinute) //Redraw der Statuszeile erzwingen:
   {
      ...
      {
         //Synchronisierung durch Robots; wird um 18 Uhr auf false gesetzt (w�rde sonst in CAbend.cpp geschehen)
         if (Sim.Players.GetAnzRobotPlayers()==0)
            for (SLONG c=0; c<4; c++)
               Sim.Players.Players[c].bReadyForMorning = true;
      }

      //Verschiedene Sync's f�r's Netzwerk:
         ...
         if (!Sim.Players.Players[c].IsOut)
         {
            ...
            if (qPlayer.StrikeHours && qPlayer.StrikeEndCountdown)
            {
               ...
               if (qPlayer.StrikeEndCountdown==0)
               {
                  ...
                  if (qPlayer.Owner==0)
                  {
                     qPlayer.StrikeNotified = FALSE;   //Dem Spieler bei n�chster Gelegenheit bescheid sagen
                     //Die Variable qPlayer.StrikeEndType mu� bereits gesetzt sein
                  }
               }
            }
      ...
      if (Minute<OldMinute)
      {
         ...
         //Probleme bei einem Flugzeug ausl�sen:
      ...
      {
         //Conditional Block: DutyFree, ArabAir, LastMinute, Museum, Reiseb�ro:
            ...
            for (d=0; d<(SLONG)qPlayer.Planes.AnzEntries(); d++)
            {
               if (qPlayer.Planes.IsInAlbum (d))
               {
                  ...
                  if (qPlane.Ort==-5 && qPlane.Flugplan.NextFlight!=-1)
                  {
                     ...
                     {
                        //Unf�lle wegen schlechter Qualit�t?
                        TEAKRAND LocalRand (Sim.Date+GetHour()+GetMinute());
                        ...
                        if (LocalRand.Rand(5000)>4980)
                        {
                           ...
                           switch (GetSeason())
                           {
                              //0-3 : Fr�hling, Sommer, Herbst, Winter:
                        ...
                        //Wir m�ssen die aktuelle L�nge/Breite berechnen:
      ...
      for (c=0; c<Sim.Players.AnzPlayers; c++)
      {
         ...
         for (d=0; d<(SLONG)Sim.Players.Players[c].Planes.AnzEntries(); d++)
         {
            if (Sim.Players.Players[c].Planes.IsInAlbum (d))
            {
               ...
               {
                   //M�ssen wir den Flug evtl. verschieben?
   ...
   //Alle Flugzeuge zum landen in Anflugposition? (Anschlie�end zum Gate zum aussteigen)
      ...
      for (c=0; c<Sim.Players.AnzPlayers; c++)
      {
         ...
         for (d=0; d<(SLONG)qPlayer.Planes.AnzEntries(); d++)
         {
            if (qPlayer.Planes.IsInAlbum (d))
            {
               ...
               //Testen, ob sie jetzt landen m�ssen:
               ...
               {   
                  ...
                  //Ersatzteil f�r reparaturbed�rftige Flugzeuge:
      ...
      for (c=0; c<Sim.Players.AnzPlayers; c++)
      {
         ...
         for (d=0; d<(SLONG)Sim.Players.Players[c].Planes.AnzEntries(); d++)
         {
            if (Sim.Players.Players[c].Planes.IsInAlbum (d))
            {
               ...
               //Testen, ob sie jetzt hinters Fenster m�ssen; dazu Flugplan durchgehen:
               if (qPlane.Flugplan.NextFlight!=-1)
                   ...
                   qPlane.Flugplan.Flug[qPlane.Flugplan.NextFlight].Startzeit==GetHour())
               {
                  ...
                  {
                     //F�llfl�ge starten direkt; andere holen erst Leute ab
...
//------------------------------------------------------------------------------
//Gibt die aktuelle Woche (1,2,3,4) zur�ck:
//------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt die aktuelle Stunde zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt die Jahrezeit zur�ck. Das ist nicht immer SIM::Jahreszeit. Die Variable enth�lt, die
...
//--------------------------------------------------------------------------------------------
//�berpr�ft, ob wir die richtigen Bricks geladen haben:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt die aktuelle Minute zur�ck:
//--------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//Gibt die aktuelle Zeit als String zur�ck:
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//L��t neuen Tag & ggf. Monat beginnen
//------------------------------------------------------------------------------
void SIM::NewDay (void)
{
   ...
   for (c=0; c<Sim.Players.AnzPlayers; c++)
      if (!Sim.Players.Players[c].IsOut)
      {
         ...
         hprintf ("- Routen: %li, Auftr�ge: %li, Fl�ge (heute): %li, Flugzeuge: %li", (LPCTSTR)Sim.Players.Players[c].RentRouten.GetNumUsed(), (LPCTSTR)Sim.Players.Players[c].Auftraege.GetNumUsed(), (LPCTSTR)Sim.Players.Players[c].NumFlights, (LPCTSTR)Sim.Players.Players[c].Planes.GetNumUsed());
      } */
   ...
   //In den Reiseb�ros die Zettel nachf�llen:
   gFrachten.Random.SRand (Sim.Date);
   LastMinuteAuftraege.Random.SRand (Sim.Date+1);
   ReisebueroAuftraege.Random.SRand (Sim.Date+2);
      ...
      for (c=0; c<7; c++)
      {
         ...
         if (TafelData.City[c].ZettelId!=-1 && TafelData.City[c].Player!=-1)
         {
            ...
            if (TafelData.City[c].WasInterested)
               if (TafelData.City[c].Player==Sim.localPlayer) GotIt+=bprintf("%s: %s�", LPCTSTR(StandardTexte.GetS (TOKEN_MISC, 2610)), LPCTSTR(Cities[TafelData.City[c].ZettelId].Name));
                                                         else GotItNot+=bprintf("%s: %s�", LPCTSTR(StandardTexte.GetS (TOKEN_MISC, 2610)), LPCTSTR(Cities[TafelData.City[c].ZettelId].Name));
         ...
         if (TafelData.Route[c].ZettelId && TafelData.Route[c].Player!=-1)
         {
            ...
            if (TafelData.Route[c].WasInterested)
               if (TafelData.Route[c].Player==Sim.localPlayer) GotIt+=bprintf("%s: %s-%s�", LPCTSTR(StandardTexte.GetS (TOKEN_MISC, 2611)), LPCTSTR(Cities[Routen[TafelData.Route[c].ZettelId].VonCity].Name), LPCTSTR(Cities[Routen[TafelData.Route[c].ZettelId].NachCity].Name));
                                                          else GotItNot+=bprintf("%s: %s-%s�", LPCTSTR(StandardTexte.GetS (TOKEN_MISC, 2611)), LPCTSTR(Cities[Routen[TafelData.Route[c].ZettelId].VonCity].Name), LPCTSTR(Cities[Routen[TafelData.Route[c].ZettelId].NachCity].Name));
         ...
         if (TafelData.Gate[c].ZettelId!=-1 && TafelData.Gate[c].Player!=-1)
         {
            ...
            if (TafelData.Gate[c].WasInterested)
               if (TafelData.Gate[c].Player==Sim.localPlayer) GotIt+=bprintf("%s: %li�", LPCTSTR(StandardTexte.GetS (TOKEN_MISC, 2612)), TafelData.Gate[c].ZettelId+1);
                                                         else GotItNot+=bprintf("%s: %li�", LPCTSTR(StandardTexte.GetS (TOKEN_MISC, 2612)), TafelData.Gate[c].ZettelId+1);
   ...
   //O1: decay only triggers once already hostile (< 0, not < 25) — 50pt buffer
   ...
   for (c=0; c<Sim.Players.AnzPlayers; c++)
      for (d=0; d<Sim.Players.AnzPlayers; d++)
      {
         ...
         //O5: alliance bonus — cooperation counters wealth envy (+1/day, caps at 70)
   ...
   //Bei ATFS-Megasabotage-Mission ggf. k�nstlich Sabotage einf�gen:
...
{ 
   ...
   if (SaveVersionSub>=100)
   {
      ...
      BOOL bIsHostDummy;
      File >> bIsHostDummy; //Sim.bIsHost; (bringt probleme, wenn wir den Host so �ndern; wir m�ssen beim DX Wert bleiben)
   }
   else
   {
...
BOOL SIM::LoadGame (SLONG Number)
{
   ...
   //Ein paar Dinge, wenn man vom Hauptmen� kommt:
...
reload_airport:
   ...
   //Postfix zum Laden von alten Spielst�nden in die Deluxe-Version:
   if (NumSaveGameCities!=(SLONG)Cities.AnzEntries())
   {
      ...
      //Die RentRouten der Players auf die Gr��e bringen und die neue Reihenfolge mappen
      for (c=0; c<Sim.Players.Players.AnzEntries(); c++)
      {
      ...
      //Die RentCities der Players auf die Gr��e bringen und die neue Reihenfolge mappen
...
//--------------------------------------------------------------------------------------------
// Gibt den Localplayer eines Spielstandes zur�ck
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
// Gibt den UniqueGameId eines Savegames zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//L�dt die Optionen aus der Registry:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Bringt den Benutzungplan der R�ume auf den neusten Stand:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
// Ein Spieler will einen anderen �bernehmen:
//--------------------------------------------------------------------------------------------
...
void SIM::AddHighscore (CString Name, DWORD UniqueGameId2, __int64 Score)
{
   ...
   //Pass 1: Einen existierenden Eintrag der gleichen Session �berschreiben?
   for (c=0; c<6; c++)
   ...
   //Pass 2: Eine neue Session hinzuf�gen:
   for (c=0; c<6; c++)
      if (Score>Highscores[c].Score)
      {
         for (d=4; d>=c; d--)
...
void COptions::ReadOptions (void)
{
   ...
   //Schwierigkeitsgrad f�r AddOn decodieren:
   {
      for (c=0; c<10; c++)
   ...
   //Schwierigkeitsgrad f�r AddOn Flight Security decodieren:
   {
      for (c=0; c<10; c++)
...
//Demo ohne Intro an. Und wenn er dann das richtige Spiel kauft g�be es keinen Intro.
#ifndef NO_INTRO
   reg.WriteRegistryKey (&OptionViewedIntro);
#endif

//Und fast das gleiche f�r die Sprechblasen
#ifdef NO_D_VOICES
...
//--------------------------------------------------------------------------------------------
//Verwaltung f�r den neuen Monat:
//--------------------------------------------------------------------------------------------
