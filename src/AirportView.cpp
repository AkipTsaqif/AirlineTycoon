//============================================================================================
// AirportView.cpp - Das Fenster zum anzeigen & handlen der isometrischen �bersicht
//============================================================================================
// Links: "AirportView.h"
//============================================================================================
#include "stdafx.h"
...
//Die statischen Daten daf�r initialisieren:
...
{
   //Per Default kein Objekt f�r den Editor angeben und keine Personen hinzuf�gen:
   ...
   //Automatisches Scrolling
	if (!gMouseScroll)
	{
		...
		else //Scrolling:
		{
            ...
            //�nderungen in der Kamera-Geschwindigkeit nur akzeptieren, wenn sie auch von Dauer sind
         ...
         //Verhindern, da� die Kamera am stehenden Spieler vorbeirauscht:
   ...
   if (!Editor)
   {
      ...
      if (Sim.FocusPerson!=-1)
         if (!Sim.Persons.IsInAlbum (Sim.FocusPerson))
         {
            ...
            //�ber gro�e Strecken lieber faden als scrollen
...
void AirportView::OnPaint() 
{
   ...
   static SLONG ParallaxIndex[5]={-1,-1,-1,-1,-1};      //Die direkten Brick Indices f�rs Paralax
   XY   &ViewPos = Sim.Players.Players[(SLONG)PlayerNum].ViewPos;
   ...
   SLONG  LogoBarIndex    = Bricks((SLONG)0x10000000+492);  //Der Logobalken �ber dem CheckIn
   ...
   if (TopWin==NULL)
   {
      ...
      if (bActive)
      {
         ...
         //Parallax Horizont mit Clipping einbauen:
         {
            ...
            for (c=1; c<Airport.ClipMarkers.AnzEntries(); c++)
            {
               ...
               if (Airport.ClipMarkers[c-1].Type!=RUNE_CLIPFULL && Airport.ClipMarkers[c].Position>=ViewPos.x)
               {
                  ...
                  {
                     case RUNE_CLIPMIDDLE: //B�ros verdecken
            ...
            while (t1<t2 && t1!=0x7fffffff)
            {
               if (Editor!=EDITOR_BUILDS || UnderCursor!=c || (FlackerCount&1)==0)
                  if (pBuilds->IsInAlbum(c) && Bricks.IsInAlbum ((*pBuilds)[c].BrickId))
                  {
                     ...
                     {
                        SLONG gm = Airport.GateMapper[(SLONG)qBuild.Par];
                        qBrick.BlitAt (PrimaryBm, 0, qBuild.ScreenPos-ViewPos+WinP1, gm!=-1 ? (Sim.Players.Players[gm].SecurityFlags&(1<<0)) : 0);
                     }
                     else if (Editor!=EDITOR_BUILDS && BrickId>=GateSmackMin && BrickId<=GateSmackMax)
                     {
                        SLONG gm = Airport.GateMapper[(SLONG)qBuild.Par];
                        if (gm!=-1 && Sim.Players.Players[gm].SecurityFlags&(1<<8))
                     ...
                     else
                     {
                        if (BrickId==AbflugIndex || BrickId==AbflugIndex2)
                        {
                           ...
                           if (gm!=-1)
                           {
                              if (Sim.Players.Players[gm].SecurityFlags&(1<<8))  phase=1;
                              if (Sim.Players.Players[gm].SecurityFlags&(1<<10)) phase=2;
                           }
               ...
               //Das hier kostet einiges an Zeit. K�nnte man optimieren: ==>+<==
         ...
         if (!IsDialogOpen() && !MenuIsOpen() && Editor==EDITOR_NONE && !MouseWait)
         {
            ...
            if (c==ROOM_WALL) c=0; //Die Kindersicherung f�r die Wall-Eigenschaft

            if (c>=ROOM_FLIGHTDISPLAY && c<ROOM_FLIGHTDISPLAY+60)
            {
               //Die Tips f�r die Flugk�rzel:
...
//--------------------------------------------------------------------------------------------
//Schickt eine Ank�ndigung weiter...
//--------------------------------------------------------------------------------------------
...
void AirportView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
   ...
   if (WinP1.x==0 && WinP1.y==0 && TopWin==NULL)
   {
      ...
      if (GetAsyncKeyState (VK_SHIFT) && !GetAsyncKeyState (VK_CONTROL))
      {
         switch (nChar)
         {
            //Neues, autonomes & selbstzerst�rendes Fenster erzeuges:
...
//--------------------------------------------------------------------------------------------
//Gibt einen zuf�lligen Geburtsort f�r einen Kunden (0) oder einen Spieler (1) zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt einen zuf�lligen Ausgang zur�ck:
//--------------------------------------------------------------------------------------------
...
nofreetravelagents:
   ...
   //Spezialfix f�r Personen zu Reiseb�ro / LastMinute
...
//--------------------------------------------------------------------------------------------
//War ein Mausklick in einer markierten Kachel-Area? Gibt 0 oder Raum zur�ck
//--------------------------------------------------------------------------------------------
SLONG AIRPORT::IsInMarkedArea (const XY &Pos)
{
   ...
   if (NumFound)
   {
      ...
      //Spezielle Marked-Areas f�r FlightDisplays �ber CheckIn und Abflug
      ...
      //Anti-Marked Areas z.B. f�r Postkasten
      if (d==255) return (0);
...
//--------------------------------------------------------------------------------------------
//Gibt die n�chste Station eines Weges zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt eine zuf�lligen (falls mehrere m�glich) Rune des Typs zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Wieviele Shops (oder Gates o.�.) gibt es von einer Sorte?:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt die Treppe zur�ck, die am n�chsten ist:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt zur�ck, welchen Parameter die Rune des Types n in der Umgebung von XY hat:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt zur�ck, welcher Brick von Sorte X dort so rumh�ngt:
//--------------------------------------------------------------------------------------------
...
void AIRPORT::Load (SLONG Hall, SLONG Level)
{
   ...
   //Und daf�r sorgen, da� nichts im negativen Bereich liegt:
...
//--------------------------------------------------------------------------------------------
//L�dt einen kompletten Flughafen indem er ihn aus Einzelteilen zusammensetzt
//--------------------------------------------------------------------------------------------
//#pragma optimize("", off)
...
void AIRPORT::LoadAirport (SLONG LeftEnd, SLONG CheckIn, SLONG Office, SLONG Entry, SLONG Shops, SLONG Cafe, SLONG Security, SLONG Suitcase, SLONG WaitZone, SLONG RightEnd)
{
   ...
   SLONG  Count[10];        //F�nf Counter, da alle f�nf Abschnitte zugleich zusammengef�gt werden
...
//--------------------------------------------------------------------------------------------
//Den Flughafen f�r einen neuen Tag vorbereiten: Personen entfernen & Spieler hinzuf�gen
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Berechnet den x-Index f�r das iPlate Array aus einer Brick-Position: (Alignment -1=l 0=c 1=r)
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Berechnet den y-Index f�r das iPlate Array aus einer Brick-Position:
//--------------------------------------------------------------------------------------------
...
SLONG AIRPORT::CalcPlateYPosition (BUILD &qBuild, SLONG BrickYOffset)
{
   ...
   //Nicht ungef�hrlich, denn das passiert noch �fter
   ...
   for (c=0; c<SLONG(Builds.AnzEntries()); c++)
   {
      if (Builds.IsInAlbum(c))
      {
         switch (Bricks[Builds[c].BrickId].ObstacleType)
         {
            ...
            //Gro�e Vertikale Abgrenzung: (ver�ndert nur die Ausg�nge einer Plate)
            ...
            //Vertikale Abgrenzung: (ver�ndert nur die Ausg�nge einer Plate)
            case OBST_VERTIKAL:
               x = CalcPlateXPosition (c, -10+4, -1);
               y = CalcPlateYPosition (c, -8);
               if (y>=0 && y<=15 && x>=0) { FUCK(y+(x<<4)); iPlate[y+(x<<4)] &= (~64); {FUCK(y+((x+1)<<4));iPlate[y+((x+1)<<4)] &= (~16); }}
            ...
            //Der Eingang zu einem Gesch�ft / Museum:
            case OBST_SHOPFRONT:
            case OBST_MUSEUM:
               x  = CalcPlateXPosition (c, 10, -1);
               x2 = CalcPlateXPosition (c, -10, 1);
               ...
               for (y--;y>=0 && y<=14 && y!=4; y--)
               {
                  if (Bricks[Builds[c].BrickId].ObstacleType==OBST_SHOPFRONT && x>=0)
                  {
                     {FUCK(y+(x<<4));}
                     {FUCK(y+((x-1)<<4));}
                     iPlate[y+((x-1)<<4)]  &= (~64);   //linke Wand (au�en)
                  ...
                  if (x2>=0)
                  {
                     {FUCK(y+(x2<<4));     }
                     {FUCK(y+((x2+1)<<4));}
                     iPlate[y+((x2+1)<<4)] &= (~16);   //rechte Wand (au�en)
            ...
            //Die Runen stellen immer Sonderf�lle dar:
            case OBST_RUNE:
               x  = CalcPlateXPosition (c, 0, 0);
               y = CalcPlateYPosition (c, 0);
...
//--------------------------------------------------------------------------------------------
//Schaut, ob die T�ren bewegt werden m�ssen:
//--------------------------------------------------------------------------------------------
void AIRPORT::PumpDoors (void)
{
   ...
   for (c=Doors.AnzEntries()-1; c>=0; c--)
   {
      ...
      {
         if (Doors[c].Dir==1) //Feuerl�scher nach innen drehen ("�ffnen")
         {
            if (Doors[c].State>=40)
            {
               Doors[c].Dir=0;
            ...
            /*if (IsRoomBusy(ROOM_SABOTAGE, -1))
            {
               //T�r resetten, weil Spieler nur halb reingegangen ist
               Doors[c].State=(10-1)*2;
               Doors[c].Dir=-1;
            } */
         ...
         else if (Doors[c].Dir==-1) //Feuerl�scher wieder nach au�en drehen ("schlie�en")
         {
            Doors[c].State--;
...
void AIRPORT::TryDoor (XY ArrayPos, BOOL Player, SLONG PlayerNum)
{
   ...
   for (c=Doors.AnzEntries()-1; c>=0; c--)
   {
      if (Doors[c].ArabDoor==2) //Frachtt�r
      {
         if (!Player) continue;
...
//--------------------------------------------------------------------------------------------
//Sorgt daf�r, da� nach neuem Tag/LoadGame eine T�r richtig (Explodiert oder nicht) angezeigt wird:
//--------------------------------------------------------------------------------------------
...
void AIRPORT::CalcCoordinates (void)
{
   ...
   //Die Area-Runen automatisch erg�nzen:
   for (c=0; c<SLONG(Builds.AnzEntries()); c++)
      if (Builds.IsInAlbum(c))
      {
   ...
   //Die Zahl der f�r uns wichtigen Runen bestimmen:
   for (c=0; c<SLONG(Builds.AnzEntries()); c++)
...
void AIRPORT::RemoveRunes (void)
{
   ...
   for (c=d=0; c<(SLONG)Builds.AnzEntries(); c++)
   {
      //Einige Runen werden zum Abschu� freigegeben:
      if (Builds.IsInAlbum(c))
         switch (Builds[c].BrickId-0x10000000)
         {
            case RUNE_UP:
...
//--------------------------------------------------------------------------------------------
//Gibt f�r die On-Screen Informationen einen STATIC Pointer auf den Filenamen zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Sorgt daf�r, da� die Builds die Bricks nicht-assoziativ adressieren; Nicht mit Airport Parts verwenden!
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Generiert die Mapper-Tabelle f�r die Schalter-Schildchen
//--------------------------------------------------------------------------------------------
void AIRPORT::CreateGateMapper (void)
{
   SLONG c, d;
   SLONG physGates = GetNumberOfShops (RUNE_2WAIT);
   SLONG AnzGates  = Sim.WaitZone >= 5 ? Sim.WaitZone + 3 : physGates;
