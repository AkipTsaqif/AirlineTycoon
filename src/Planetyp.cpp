...
void CPlaneTypes::ReInit (const CString &TabFilename)
{
   ...
   while (1)
   {
      ...
      //Hinzuf�gen (darf noch nicht existieren):
...
//--------------------------------------------------------------------------------------------
//Sucht einen zuf�lligen (bereits erfundenen) Flugzeugtyp raus:
//--------------------------------------------------------------------------------------------
ULONG CPlaneTypes::GetRandomExistingType (TEAKRAND *pRand, SLONG maxErstbaujahr)
{
   SLONG c, Anz, rnd;

   for (c=Anz=0; c<(SLONG)AnzEntries(); c++)
      if (IsInAlbum (c))
      if ((*this)[c].Erstbaujahr <= maxErstbaujahr)
...
void CPlaneTypes::BlitPlaneAt (SBPRIMARYBM &TargetBm, SLONG PlaneType, SLONG Size, XY Pos, SLONG OwningPlayer)
{
   switch (Size)
   {
      ...
      case 2:
         {
            //Breite f�r's zentrieren berechnen:
...
//--------------------------------------------------------------------------------------------
//Bewegt das Flugzeug f�r Animationen etwas weiter:
//--------------------------------------------------------------------------------------------
   ...
   switch (Ort)
   {
      ...
      case -3:
         SkipPlaneCalculation=0;
         {
            ...
            //Lautst�rke f�r Flugzeuge berechnen:
            ...
            if (AirportPos.x>TargetX)
            {
               ...
               if (AirportPos.x<=TargetX) //Ist das Flugzeug angekommen?
               {
                  ...
                  else  //nein, abholen
                  {
                     ...
                     //Personen brauchen nicht mehr warten, sondern k�nnen das Flugzeug st�rmen:
      ...
      //Am Boden (in einer Stadt) ==> M�ssen wir es ausr�sten?
      default:
...
//--------------------------------------------------------------------------------------------
//�berpr�ft, die Flugpl�ne f�r ein Flugzeug und streicht ggf. Fl�ge
//--------------------------------------------------------------------------------------------
void CPlane::CheckFlugplaene (SLONG PlayerNum, BOOL Sort, BOOL PlanGates)
{
   ...
   //Automatikfl�ge l�schen, andere erg�nzen:
   for (c=Flugplan.Flug.AnzEntries()-1; c>=0; c--)
   ...
   //N�tigenfalls zwischendurch automatische Fl�ge einbauen:
   for (c=0; c<Flugplan.Flug.AnzEntries()-1; c++)
   {
      ...
      if (VonCity!=NachCity)
      {
         ...
         //Pr�fen, ob Startzeit- und Datum auch in der Zukunft liegen:
   ...
   //�bersch�ssige Fl�ge abschneiden:
   ...
   //Automatik-Fl�ge m�glichst sp�t ansetzen:
   for (c=0; c<Flugplan.Flug.AnzEntries()-1; c++)
   {
      if (Flugplan.Flug[c+1].ObjectType==0) break;
...
//--------------------------------------------------------------------------------------------
//Setzt den Flugplan auf Knopfdruck f�r die restliche Woche fort:
//--------------------------------------------------------------------------------------------
void CPlane::ExtendFlugplaene (SLONG PlayerNum)
{
   SLONG c;
   SLONG RapportL;    //L�nge des Rapports in Tagen
...
void CPlane::FlugplaeneFortfuehren (SLONG PlayerNum)
{
   ...
   //Den letzten Flug als Ma�stab heraussuchen:
   for (c=0; c<Flugplan.Flug.AnzEntries(); c++)
   {
   ...
   for (c=0; c<Flugplan.Flug.AnzEntries(); c++)
   {
      ...
      {
         //Alte Auftr�ge oder Automatikfl�ge l�schen:
         ...
         //Ist der Flug trotz Verschiebung noch von gestern? Dann l�schen:
...
//--------------------------------------------------------------------------------------------
//Kann das Flugzeug zur Zeit verkauft werden, oder sind Fl�ge darauf gebucht?
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
// Gibt die gr��te Passagieranzahl eines noch offenen Fluges zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//F�gt einem Flugzeug eine Panne hinzu:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Bewegt alle Flugzeuge f�r Animationen etwas weiter:
//--------------------------------------------------------------------------------------------
...
void CPlaneNames::ReInit (const CString &TabFilename)
{
   ...
   //Keine freien Pl�tze offen lassen:
