...
void PLAYER::ChangeMoney (__int64 Money, SLONG Reason, CString Par1, char *Par2)
{
   ...
   if (Money>0 && (PLAYER::Money+Money)<PLAYER::Money)
      ; //Nichts machen, weil's sonst einen Overflow geben w�rde
   else
      PLAYER::Money += Money;
...
//--------------------------------------------------------------------------------------------
//Geht in einen Raum hinein (Fenster wird er�ffnet)
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt den aktuellen Raum zur�ck:
//--------------------------------------------------------------------------------------------
...
void PLAYER::CalcRoom (void)
{
   ...
   //Z�hlen, wie lange wir in diesem Raum schon sind:
   for (c=9; c>=0; c--)
      if (Locations[c])
      {
...
//--------------------------------------------------------------------------------------------
//Berechnet, was die anstehenden Umr�stungen zusammen kosten werden:
//--------------------------------------------------------------------------------------------
...
long PLAYER::CalcSecurityCosts (bool bFixOnly, bool bPlaneOnly)
{
   ...
   if (SecurityFlags & 0x0001) costfix   +=   25000; //B�ro
   if (SecurityFlags & 0x0002) costfix   +=   20000; //Laptop
   if (SecurityFlags & 0x0004) costfix   +=   25000; //Personb�ro
...
//--------------------------------------------------------------------------------------------
//Zieht Miete f�r Cities und Routen vom Geld ab:
//--------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//Gibt den aktuellen Missionserfolg zur�ck:
//------------------------------------------------------------------------------
         ...
         case DIFF_ADDON01:
            {
               ...
               if (Money>0) rc=Credit;         //Nur Kredit
                       else rc=Credit-Money;   //Kredit + �berziehungskredit
...
void PLAYER::NewDay (void)
{
   ...
   //Aktienkurseintr�ge verschieben:
   for (c=9; c>=1; c--)
      Kurse[c]=Kurse[c-1];
   ...
   //Den Flugplan f�r die Routen updaten:
   if (DoRoutes) RobotPlanRoutes ();

   //Eine Verm�genssteuer. Speziell f�r Eric:
   __int64 Tax=0;

   //MaxInt64: 18446744073709551615

   //F�r ATP �berall noch 0 Nullen hinzugef�gt:
   if (Money>  50000000000000) Tax+=(Money-  50000000000000)/10;   //10% f�r Verm�gen �ber  50000 Mio
   if (Money> 100000000000000) Tax+=(Money- 100000000000000)/10;   //nochmal 10% f�r �ber  100000 Mio
   if (Money> 200000000000000) Tax+=(Money- 200000000000000)/10;   //nochmal 10% f�r �ber  200000 Mio
   if (Money> 400000000000000) Tax+=(Money- 400000000000000)/10;   //nochmal 10% f�r �ber  400000 Mio
   if (Money> 800000000000000) Tax+=(Money- 800000000000000)/10;   //nochmal 10% f�r �ber  800000 Mio
   if (Money>1200000000000000) Tax+=(Money-1200000000000000)/10;   //nochmal 10% f�r �ber 1200000 Mio
   if (Money>1600000000000000) Tax+=(Money-1600000000000000)/10;   //nochmal 10% f�r �ber 1600000 Mio
   if (Money>1700000000000000) Tax+=(Money-1700000000000000)/10;   //nochmal 10% f�r �ber 1700000 Mio
   if (Money>1800000000000000) Tax+=(Money-1800000000000000)/10;   //nochmal 10% f�r �ber 1800000 Mio
   if (Money>1900000000000000) Tax+=(Money-1900000000000000)/10;   //nochmal 10% f�r �ber 1900000 Mio
...
//--------------------------------------------------------------------------------------------
//Bringt Auftr�ge aus neusten Stand; bucht Strafen
//--------------------------------------------------------------------------------------------
void PLAYER::UpdateAuftraege (void)
{
   ...
   //Das gleiche f�r Frachtauftr�ge:
...
//--------------------------------------------------------------------------------------------
//Wurde ein Auftrag schon irgendwo eingetragen? Gibt ggf. Namen zur�ck
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Der Spieler kann in mehreren R�umen zugleich sein (B�ro, Scheduler, ...); Ist er im Parameter-Raum?
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Hat der Spieler einen bestimmten Berater? Gibt ggf. die Qualit�t zur�ck
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Welche Auftr�ge wurden wie oft verplant?:
//--------------------------------------------------------------------------------------------
void PLAYER::UpdateAuftragsUsage (void)
{
   ...
   for (c=0; c<(SLONG)Planes.AnzEntries(); c++)
      if (Planes.IsInAlbum(c))
      {
         ...
         for (d=Planes[c].Flugplan.Flug.AnzEntries()-1; d>=0; d--)
         {
            //Nur bei Auftr�gen von menschlichen Spielern
            ...
            //Frachtauftr�ge werden hier nicht behandelt
...
//--------------------------------------------------------------------------------------------
//Welche Frachtauftr�ge wurden wie oft verplant?:
//--------------------------------------------------------------------------------------------
void PLAYER::UpdateFrachtauftragsUsage (void)
{
   ...
   //TonsOpen bei allen Frachtauftr�ge resetten:
   ...
   //TonsOpen bei allen Frachtauftr�ge neu berechnen:
   for (c=0; c<(SLONG)Planes.AnzEntries(); c++)
      if (Planes.IsInAlbum(c))
      {
         CFlugplan *Plan = &Planes[c].Flugplan;
         ...
         for (d=0; d<Planes[c].Flugplan.Flug.AnzEntries(); d++)
         {
            ...
            if (qFPE.ObjectType==4)
            {
                        ...
                        qFPE.Startdate>Sim.Date || (qFPE.Startdate==Sim.Date && qFPE.Startzeit>Sim.GetHour())) 
               {
                  ...
                  //Ist dieser Frachtflug �berhaupt noch zu erledigen?
                  if (qFracht.TonsLeft)
                  {
                     //Wir misbrauchen bei Frachtfl�gen das Passagierfeld um zu speichern, wieviel Fracht hier mitfliegt
               ...
               if (Plan->Flug[d].VonCity==(ULONG)Sim.HomeAirportId || Plan->Flug[d].NachCity==(ULONG)Sim.HomeAirportId)
               {
                  ...
                  if (tmp>1 && tmp<24*7-1)
                  {
                     ...
                     if (Plan->Flug[d].VonCity==ULONG(Sim.HomeAirportId))
                     {
                        ...
                        {
                           //Gate wird f�r Flug veranschlagt:
                           if (Plan->Flug[d].Startdate>Sim.Date || (Plan->Flug[d].Startdate==Sim.Date && Plan->Flug[d].Startzeit>Sim.GetHour()+1))
...
//--------------------------------------------------------------------------------------------
//L��t einen Spieler zu einer Rune laufen: rc=FALSE, wenn Raum geschlossen
//--------------------------------------------------------------------------------------------
BOOL PLAYER::WalkToRoom (UBYTE RoomId)
{
   ...
   //Schauen, ob wir zu dem Raum gehen k�nnen, oder ob zu ist:
   SLONG MenuClosed=-1;
...
//--------------------------------------------------------------------------------------------
//L��t den Spieler zu einer bestimmten, angeklickten Stelle laufen:
//--------------------------------------------------------------------------------------------
void PLAYER::WalkToMouseClick (XY AirportClickPos)
{
   ...
   if (Room==ROOM_WALL)
   {
      //Bei Klick auf die Wand eines B�ros davor stehenbleiben
   ...
   else
   {
      ...
      //Test: Ist Zielkachel blockiert? Dann evtl. s�dlich davon!
...
//--------------------------------------------------------------------------------------------
//L��t den Spieler zu einer bestimmten Stelle laufen:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Der Spieler l�uft nicht mehr automatisch:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Berechnet das Sekund�rziel f�r den Spieler:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//L��t den Spieler in die richtige Richtung laufen:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Erledigt Dinge f�r den Roboter:
//--------------------------------------------------------------------------------------------
void PLAYER::RobotPump()
{
   ...
   if (pSmack)
      if (!pSmack->Next (&Bricks[SLONG(0x10000000+734+PlayerNum)].Bitmap[0]))
      {
         ...
         //Alle Figuren in der N�he der T�r "entminen"
   ...
   if (SickTokay && !RunningToToilet && !IsStuck)
   {
      ...
      if (LocationWin==NULL || !((CStdRaum*)LocationWin)->MenuIsOpen())
      {
         ...
         if (PlayerExtraRandom.Rand(4)==0)
         {
            ...
            //Nur wenn Spieler nicht gerade Raum betritt oder verl��t und auch nicht auf Toilette ist:
...
void PLAYER::RobotPlan()
{
   ...
   if (RobotActions[0].ActionId==ACTION_NONE && RobotActions[1].ActionId==ACTION_NONE)
   {
      //Zeit f�r default-Actions?
      // Compute safe reserve: keep 2 months of route rent as buffer before buying a plane
...
void PLAYER::RobotPlanRoutes(void)
{
   ...
   forall (c, Planes)
      if (Planes.IsInAlbum(c))
      {
         ...
         for (d=qPlan.Flug.AnzEntries()-1; d>=0; d--)
         {
            ...
            //Was fliegen wir dann �berhaupt noch?
            ...
            if (Planes.IsInAlbum(d))
            {
               ...
               for (e=0; e<qPlan.Flug.AnzEntries(); e++)
               {
                  if (qPlan.Flug[e].ObjectType==0)
                  {
                     ...
                     Planes[d].CheckFlugplaene(PlayerNum, TRUE, FALSE);  //FALSE=Gates erst sp�ter planen
   ...
   //Test: Haben wir f�r jede Route ein Flugzeug, was gro� genug ist?
   BuyBigPlane=0;
   for (c=0; c<SLONG(Routen.AnzEntries()); c++)
      if (Routen.IsInAlbum(c) && RentRouten.RentRouten[c].Rang)
   ...
   do
   {
      ...
      for (c=0; c<SLONG(Routen.AnzEntries()); c++)
         ...
         {
            if (BestC==-1 || (Routen[c].Bedarf/(PlanesOnRoute[c]+1)>Routen[BestC].Bedarf/(PlanesOnRoute[BestC]+1)))
               BestC=c;
         }
      ...
      if (BestD!=-1)
      {
         ...
         for (d=0; d<qPlan.Flug.AnzEntries(); d++)
         {
            if (qPlan.Flug[d].ObjectType==0)
            {
               ...
               Planes[BestD].CheckFlugplaene(PlayerNum, TRUE, FALSE);  //FALSE=Gates erst sp�ter planen
   ...
   for (c=0; c<SLONG(Routen.AnzEntries()); c++)
      if (Routen.IsInAlbum(c) && RentRouten.RentRouten[c].Rang)
      {
         ...
         // Include route rent in the cost floor so pricing covers total operating cost
         SLONG FlightsPerMonth = max(1L, 30L / max(1L, (SLONG)RentRouten.RentRouten[c].AvgFlown));
         SLONG RentPerPax      = RentRouten.RentRouten[c].Miete / max(1L, FlightsPerMonth * max(1L, (SLONG)Routen[c].AnzPassagiere()));
         RentPerPax            = min(RentPerPax, Cost); // cap: rent adjustment cannot exceed fuel cost
         SLONG TotalCost       = Cost + RentPerPax;
         ...
         if (e>1)
         {
            ...
            RentRouten.RentRouten[c].Ticketpreis   = max (RentRouten.RentRouten[c].Ticketpreis, TotalCost/2/10*10);
            RentRouten.RentRouten[c].TicketpreisFC = max (RentRouten.RentRouten[c].TicketpreisFC, TotalCost/2/10*10*2);

            //Kein Preiskampf bei Finanzkrise:
            if (Money<100000)
            {
               RentRouten.RentRouten[c].Ticketpreis   = max (RentRouten.RentRouten[c].Ticketpreis, TotalCost/10*10);
               RentRouten.RentRouten[c].TicketpreisFC = max (RentRouten.RentRouten[c].TicketpreisFC, TotalCost/10*10*2);
            }
            if (Money<0)
            {
               RentRouten.RentRouten[c].Ticketpreis   = max (RentRouten.RentRouten[c].Ticketpreis, TotalCost*2/10*10);
            ...
            if (RentRouten.RentRouten[c].Ticketpreis < TotalCost/10*10)
         ...
         else // e==1: AI is the only operator on this route — still check for structural losses
         {
            if (RentRouten.RentRouten[c].Ticketpreis < TotalCost/10*10)
               RentRouten.RentRouten[c].TageMitVerlust++;
            else if (RentRouten.RentRouten[c].TageMitVerlust > 0)
               RentRouten.RentRouten[c].TageMitVerlust--;

            if (RentRouten.RentRouten[c].TageMitVerlust>20 && !RobotUse(ROBOT_USE_NEVERGIVEROUTEUP))
            {
               if (Sim.Players.Players[Sim.localPlayer].HasBerater (BERATERTYP_INFO))
               {
                  Sim.Players.Players[Sim.localPlayer].Messages.AddMessage (BERATERTYP_INFO, bprintf (StandardTexte.GetS (TOKEN_ADVICE, 9006), (LPCSTR)NameX, (LPCSTR)AirlineX, (LPCTSTR)Cities[Routen[c].VonCity].Name, (LPCTSTR)Cities[Routen[c].NachCity].Name));
...
//--------------------------------------------------------------------------------------------
//Computerspieler ist angekommen, jetzt wird Aktion ausgef�hrt:
//--------------------------------------------------------------------------------------------
void PLAYER::RobotExecuteAction(void)
{
   ...
   if (Sim.bNetwork)
   {
      //Synchronisieren (tags�ber):
      if (WaitWorkTill==-1)
      {
         if (Sim.bIsHost)
         {
            //Wird das zu sp�t (Wegen der 18-Uhr-Grenze)?
            ...
            if (!(Sim.CallItADay==1))
            {
               //Normal tags�ber synchronisieren:
               WaitWorkTill  = Sim.TimeSlice+40;  //Zwei Sekunden warten, bis die Aktion wirklich ausgef�hrt wird
               WaitWorkTill2 = Sim.Time+2000;     //Zwei Sekunden warten, bis die Aktion wirklich ausgef�hrt wird
         ...
         {
            //F�r Synchronit�t zwischen Host und Client
            if (!(Sim.CallItADay==1))
               return;
         }
      }
      ...
      if (Sim.bNetwork && Sim.GetHour()>=9 && Sim.GetHour()<18 && Sim.CallItADay==1)
      {
         ...
         if (Sim.bIsHost)
         {
            ...
            //Darauf warten, da� die anderen Spieler ihr okay geben:
         ...
         {
            //Okay, habe den Computerspieler durchgef�hrt:
   ...
   //Manchmal f�hrt der Client Sachen doppelt aus. Seltsam aber wahr. Hiermit wird's verhindert:
   {
   ...
   else LocalRandom.Rand(2);  //Sicherheitshalber, damit wir immer genau ein Random ausf�hren
   ...
   //6.3.02 Manchmal kommen wir als Client hier an, bevor in der Anrufenden Funktion der Shift durchgef�hrt wurde. Das holen wir dann hier mal schnell nach:
   if (RobotActions[0].ActionId==ACTION_NONE && RobotActions[1].ActionId!=ACTION_NONE)
   {
      //Alles in der Queue einen weiterschieben
      for (c=0; c<RobotActions.AnzEntries()-1; c++)
   ...
   //Die exakte Zeit des Ausf�hrens auf dem Server simulieren
   ...
   switch (RobotActions[0].ActionId)
   {
      ...
      case ACTION_STARTDAY:
         //Logik f�r wechsel zu Routen und sparen f�r Rakete oder Flugzeug:
         if (!DoRoutes)
         {
                     ...
                     if (Planes.IsInAlbum(c))
                     {
                        ...
                        for (level=0; level<=2; level++)
                        {
                           ...
                           if (VonZeit>=0 && VonZeit<23+24+24)
                           {
                              if (level==0 || level==1)
                              {
                                 for (e=qFrachten.AnzEntries()-1; e>=0; e--)
                                 {
                                    ...
                                    if (qFracht.Praemie>=0 && qFracht.VonCity==ULONG(VonCity))
                                    {
                                       ...
                                       //Multiplikator, weil wir mehrere Fl�ge durchf�hren m�ssen:
                              ...
                              {
                                 SLONG MinMin=0;  //Min mu� mindestens so gro� sein
      ...
      case ACTION_SABOTAGE:
         if (dislike==-1 && (RobotUse(ROBOT_USE_EXTREME_SABOTAGE) || LocalRandom.Rand(3)==0 || PlayerNum==0))
         {
            ...
            {
               long r=-9999; //Besten Spieler als Sabotageziel w�hlen:
               for (long c=0; c<4; c++)
      ...
      case ACTION_VISITAUFSICHT:
         ...
         {
            SLONG nExpandCap = Sim.Options.OptionExpandAirport ? LONG_MAX : 5;
      ...
      case ACTION_VISITROUTEBOX:
         ...
         do
         {
            ...
            //Mal schauen wie unser Routen/Flugzeug Verh�ltnis aussieht:
            ...
            if ((DoRoutes || WantToDoRoutes) && SLONG(Planes.GetNumUsed())>(Anz/2)*3/2 && Anz<SLONG(Routen.GetNumUsed()))
            {
               ...
               //Mal sehen, was wir kaufen k�nnen:
               ...
               for (c=0; c<RentRouten.RentRouten.AnzEntries(); c++)
               {
                  if (IsBuyable[c])
                  {
                     //Ist die Route f�r die Mission wichtig?
                     ...
                     {
                        // Score by estimated profit: revenue minus fuel and rent per flight
                        SLONG fuelCost   = CalculateFlightCost(Routen[c].VonCity, Routen[c].NachCity, 800, 800, -1)*3/180;
                        SLONG projPax    = max(1L, (SLONG)Routen[c].Bedarf / 4); // ~25% market share
                        SLONG revenue    = fuelCost * 2 * 3 * projPax;
                        SLONG rentPerFlt = max(1L, RentRouten.RentRouten[c].Miete / 20);
                        SLONG score      = revenue - fuelCost - rentPerFlt;
                        if (BestC==-1 || score > Best)
                        {
                           Best  = score;
                           BestC = c;
                        }
   ...
   //Die exakte Zeit des Ausf�hrens auf dem Server simulieren (Ende):
...
//--------------------------------------------------------------------------------------------
//Verz�gert Fl�ge um Gates auszulasten:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Der Berater gibt Nachrichten f�r zwischendurch aus:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Hat der Spieler noch Platz f�r ein Item?
//--------------------------------------------------------------------------------------------
...
void PLAYER::MapWorkerOverflow (BOOL Advice)
{
   ...
   //Z�hlung in den Flugzeugen vorbereiten:
   for (c=0; c<SLONG(Planes.AnzEntries()); c++)
      if (Planes.IsInAlbum (c))
      {
...
void PLAYER::MapWorkers (BOOL Advice)
{
   ...
   //Sind irgendwo mehr Stewardessen als gew�nscht?:
   for (c=0; c<SLONG(Planes.AnzEntries()); c++)
...
again: //Zweiter Pass, wenn Stewardessen aus Luxusstellen umgebucht werden, damit Basisstellen besetzt werden
   ...
   for (c=0; c<Workers.Workers.AnzEntries(); c++)
      if (Workers.Workers[c].Employer==PlayerNum && (Workers.Workers[c].Typ==WORKER_PILOT || Workers.Workers[c].Typ==WORKER_STEWARDESS) && Workers.Workers[c].PlaneId==-1)
      {
         ...
         else //Kein Flugzeug gefunden? F�r Stewardessen kann man noch was machen:
         {
            for (d=0; d<SLONG(Planes.AnzEntries()); d++)
               if (Planes.IsInAlbum (d))
   ...
   //Brauchen wir jetzt noch Begleiter? Und haben wir au�erdem irgendwo mehr als unbedingt notwendig?
   {
   ...
   //Und jetzt noch die offenen Stellen z�hlen:
   for (d=0; d<SLONG(Planes.AnzEntries()); d++)
      if (Planes.IsInAlbum (d))
      {
...
//--------------------------------------------------------------------------------------------
//L��t den Bodyguard ggf. einen Rabatt aushandeln:
//--------------------------------------------------------------------------------------------
...
void PLAYER::UpdateStatistics (void)
{
   ...
   if (Money>0) Statistiken[STAT_KREDIT].SetAtPastDay (0, Credit);         //Nur Kredit
           else Statistiken[STAT_KREDIT].SetAtPastDay (0, Credit-Money);   //Kredit + �berziehungskredit
...
//--------------------------------------------------------------------------------------------
//�berpr�ft alle Flugpl�ne auf tote Eintr�ge: (crasht bei toten Eintr�gen; nur zum testen)
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt den Index des x. menschlichen (Nicht-Computer) Spielers zur�ck:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//L��t alle Spieler zum Raum der Flughafenaufsicht laufen:
//--------------------------------------------------------------------------------------------
...
void PLAYERS::RobotPump()
{
   ...
   //Hack: Ab & Zu, wenn alle Player im Flughafen sind und keiner telefoniert, werden alle R�ume freigegeben...
   if (!Sim.CallItADay)
   {
...
//--------------------------------------------------------------------------------------------
//Bit 0: Handy, Bit 1: Ferngespr�ch
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Aktualisiert die Kosten f�r Gespr�che
//--------------------------------------------------------------------------------------------
...
TEAKFILE &operator >> (TEAKFILE &File, PLAYER &Player)
{
   ...
   //Mehr Strecken im AddOn ==> RentCities ggf. erg�nzen
...
//--------------------------------------------------------------------------------------------
//Unterst�tzen die Roboter im aktuellen die Level ein bestimmtes Feature?:
//--------------------------------------------------------------------------------------------
bool RobotUse (SLONG FeatureId)
{
   ...
   //Tabelle ermitteln f�r welche Level ein Feature ermittelt wird:
